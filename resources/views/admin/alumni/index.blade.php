...
<div class="dashboard-container">
    ...
    <div style="display:flex; gap:10px; margin-bottom:25px; flex-wrap:wrap;">
        <a href="#pending" style="padding:8px 20px; background:#841c26; color:white; border-radius:6px; text-decoration:none; font-weight:bold;">
            <i class="fas fa-clock"></i> Pending
            <span style="background:white;color:#841c26;border-radius:10px;padding:1px 7px;font-size:12px;margin-left:5px;">{{ $pending->total() }}</span>
        </a>
        <a href="#approved" style="padding:8px 20px; background:#27ae60; color:white; border-radius:6px; text-decoration:none; font-weight:bold;">
            <i class="fas fa-check-circle"></i> Approved ({{ $approved->total() }})
        </a>
        <a href="#rejected" style="padding:8px 20px; background:#c0392b; color:white; border-radius:6px; text-decoration:none; font-weight:bold;">
                ...
                <table class="alumni-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-family:monospace; font-size:13px; color:var(--text-secondary);">{{ $alumni->user->kpmim_id ?? '—' }}</td>
                            ...
                            <td style="display:flex; gap:8px; flex-wrap:wrap;">
                                <form method="POST" action="{{ route('admin.alumni.approve', $alumni) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="button" style="padding:6px 14px; font-size:13px; text-transform:none;">
                                        <i class="fas fa-check"></i> Approve
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.alumni.reject', $alumni) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="button" style="padding:6px 14px; font-size:13px; text-transform:none; background:#c0392b;">
